...
const Shipping = ({ history }) => {
  ...
  useEffect(() => {
    const getCity = async () => {
      ...
      if (city && data) {
        ...
        if (item && item.code) {
          const resDistrict = await axios.get(`https://provinces.open-api.vn/api/p/${item.code}?depth=2`)
          ...
          if (lDistrict.length > 0 && district) {
            ...
            if (itemDistrict && itemDistrict.code) {
              const resWard = await axios.get(`https://provinces.open-api.vn/api/d/${itemDistrict.code}?depth=2`)
  ...
  useEffect(() => {
    const getDistrict = async () => {
      const res = await axios.get(`https://provinces.open-api.vn/api/p/${code.city}?depth=2`)
  ...
  useEffect(() => {
    const getWard = async () => {
      const res = await axios.get(`https://provinces.open-api.vn/api/d/${code.district}?depth=2`)
  ...
  const shippingSubmit = (e) => {
    ...
    if (!regex.test(phoneNo)) {
      alert.error("Số điện thoại không hợp lệ (độ dài từ 8 - 11 ký tự, không chứa ký tự đặc biệt và khoảng trắng)");
      return;
    }
      ...
      <div className="shippingContainer">
        <div className="shippingBox">
          ...
          >
            ...
            <div>
              <LocationCityIcon />
              {loading ? <CircularProgress /> : <select name="city" onChange={handleChangeCity} required>
                <option value={""} selected>Chọn thành phố</option>
                {
                  listCity?.map((c) => {
                    return (
                      <option value={c.name + "-" + c.code} selected={c.name === city ? true : ""}>{c.name}</option>
                    )
                  })
                }
              </select>}
            ...
            <div>
              <LocationCityIcon />
              {loading ? <CircularProgress /> : <select name="district" onChange={handleChangeDistrict} required>
                <option value={""} selected>Chọn quận huyện</option>
                {
                  listDistrict?.map((d) => {
                    return (
                      <option value={d.name + "-" + d.code} selected={d.name === district ? true : ""}>{d.name}</option>
                    )
                  })
                }
              </select>}
            ...
            <div>
              <LocationCityIcon />
              {loading ? <CircularProgress /> : <select name="ward" onChange={handleChangeWard} required>
                <option value={""} selected>Chọn phường xã</option>
                {
                  listWard?.map((w) => {
                    return (
                      <option value={w.name + "-" + w.code} selected={w.name === ward ? true : ""}>{w.name}</option>
                    )
                  })
                }
              </select>}
